...
const services = [
  { title: 'Road Transportation', desc: 'Full Truck Load (FTL) and Less than Truck Load (LTL) scheduled network infrastructure across tier-1 corridors.', icon: Truck, size: 'md:col-span-8' },
  { title: 'Freight Forwarding', desc: 'Reliable coordination global multi-modal routes.', icon: Globe, size: 'md:col-span-4' },
  { title: 'Smart Warehousing', desc: 'Strategic high-grade fulfillment hubs packed with tracking technology and optimized inventory indexing.', icon: Layers, size: 'md:col-span-4' },
  { title: 'Last Mile Delivery', desc: 'Hyper-localized high frequency parcel distribution infrastructure built for quick SLA closeouts.', icon: Clock, size: 'md:col-span-8' },
  { title: 'Supply Chain Solutions', desc: 'End-to-end bespoke logistical diagnostic architecture engineered for asset heavy industries.', icon: BarChart3, size: 'md:col-span-6' },
  { title: 'International Cargo', desc: 'Cross-border secure transit handling with absolute clarity on legal clearance and real-time custom statuses.', icon: Shield, size: 'md:col-span-6' },
];
  ...
  return (
    <section id="services" className="py-32 px-4 max-w-7xl mx-auto relative z-10">
      <div
        ...
        style={{
          backgroundImage: 'repeating-linear-gradient(45deg, rgba(255,255,255,0.015) 0px, rgba(255,255,255,0.015) 1px, transparent 1px, transparent 32px)',
        }}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        {services.map((srv, index) => (
          ...
          >
            ...
            <div>
              ...
              <h3 className="text-xl font-bold mb-3 relative z-10 text-white group-hover:text-brand-orange transition-colors">{srv.title}</h3>
        ...
        <motion.div
          ...
          style={{ backgroundColor: 'rgba(3,7,18,0.45)' }}
        >
          <div className="grid grid-cols-1 lg:grid-cols-12 items-center gap-6 p-6 md:p-8">
            ...
            <div className="lg:col-span-8 rounded-xl overflow-hidden border border-white/5" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
              <img src="/img3.jpg" alt="Fleet" loading="lazy" className="w-full h-auto object-cover max-h-[240px]" />
            </div>
          </div>
        </motion.div>
      </div>
